angular.module('contestControllers').controller('StockPriceDeviationController', ['$scope', '$resource', 'MenuItem', function ($scope, $resource, MenuItem) {

	MenuItem.setKey(7);


	$scope.title = 'Поиск аномальных скачков курса акций.';
	$scope.dataResource = $resource('http://test.iliushin.com:8081/api/analyse?query=exec dbo.[spt_GetStockPriceDeviation] :th,\':date1\',\':date2\'');				
	...
	$scope.getTable = function() {
		if ($scope.growSize && $scope.fromDate && $scope.tillDate) {
