...
angular.module('contestControllers').controller('CompanyCapitalizeFixDollarController', ['$scope', '$resource', 'MenuItem', function ($scope, $resource, MenuItem) {
	...
	$scope.title = 'Влияние стоимости нефти на капитализацию компании при неизменности других факторов';
	$scope.dataResource = $resource('http://test.iliushin.com:8081/api/analyse?query=exec dbo.spt_AnalyzeOilRelation :start, :step, :end, \':companyName\'');				
	...
	$scope.getData = function() {
		if ($scope.step && $scope.oilStart && $scope.oilEnd && $scope.curComp) {
			var datasetY = [];
			var datasetCap = [];
			var datasetStock = [];
			...
			if (+$scope.oilEnd < (+$scope.oilStart)) {
				$scope.oilStart = [$scope.oilEnd, $scope.oilEnd=$scope.oilStart][0];
			}
			$scope.dataResource.query({end: $scope.oilEnd ,start: $scope.oilStart, step: $scope.step, companyName: $scope.curComp.value})
